...
public final class ModalBoxI<T> implements ModalAction<T> {
    ...
    public boolean isValid(ModalNaturalDeduction<T> pf) {
        ...
        LabeledProofStep<T> log = pf.getSteps().get(pf.getSteps().size() - lastAssumption);
        if (!(log.getStep() instanceof LessEqual)) {
            return false;
        }
        T stateGreater = ((LessEqual<T>) log.getStep()).getRight();
        LabeledProofStep<T> conclusion = pf.getSteps().get(pf.getSteps().size() - 1);
        if (!(conclusion.getStep() instanceof ModalLogicalOperation)) {
            return false;
        }
        if (!(pf.getSteps().get(pf.getSteps().size() - 1)).getState().equals(Optional.of(stateGreater))) {
            return false;
        }
    ...
    public void apply(ModalNaturalDeduction<T> pf) {
        ...
        applyStepSupplier(pf, (assLevel, log1, reason) -> new LabeledProofStep<>(assLevel, stateLess, (ModalLogicalOperation) log1, reason));
    }
}
