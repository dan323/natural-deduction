...
import com.dan323.proof.generic.bean.ParseAction;
...
public final class ParseModalAction {
    ...
    public static <T> List<AbstractModalAction<T>> parse(ModalNaturalDeduction<T> pf) {
        return ParseModalAction.<T>parseAction().translateToActions(pf);
    }

    public static <T> ParseAction<AbstractModalAction<T>, ModalNaturalDeduction<T>> parseAction(){
        return new ParseAction<>() {
            ...
            @Override
            public AbstractModalAction<T> parse(ModalNaturalDeduction<T> proof, int pos) {
                return ParseModalAction.parse(proof, pos);
            }
        };
    }
    ...
    public static <T> AbstractModalAction<T> parse(ModalNaturalDeduction<T> proof, int pos) {
        return parseWithReason(proof, proof.getSteps().get(pos - 1).getStep(), proof.getSteps().get(pos - 1).getProof(), proof.getSteps().get(pos - 1).getState().orElse(null));
    }
