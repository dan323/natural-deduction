...
public final class ModalDiaE<T> implements ModalAction<T> {
    ...
    public boolean isValid(ModalNaturalDeduction<T> pf) {
        ...
        T origin = pf.getSteps().get(j - 1).getState().get();
        ...
        LabeledProofStep<T> log1 = pf.getSteps().get(pf.getSteps().size() - i);
        LabeledProofStep<T> log2 = pf.getSteps().get(pf.getSteps().size() - i - 1);
        ...
        if (!(log1.getState().equals(Optional.of(rightState)) && origin.equals(leftState))) {
            return false;
        }
        if (pf.stateIsUsedBefore(log1.getState().get(), pf.getSteps().size() - i - 1)) {
            return false;
        }
    ...
    private boolean isNotFresh(ModalNaturalDeduction<T> pf, int i) {
        if ((pf.getSteps().get(pf.getSteps().size() - 1).getStep()) instanceof ModalLogicalOperation) {
            T last = (pf.getSteps().get(pf.getSteps().size() - 1)).getState().get();
    ...
    public void applyStepSupplier(ModalNaturalDeduction<T> pf, ProofStepSupplier<ModalOperation, LabeledProofStep<T>> supp) {
        LabeledProofStep<T> psm = pf.getSteps().get(pf.getSteps().size() - 1);
    ...
    public void apply(ModalNaturalDeduction<T> pf) {
        LabeledProofStep<T> psm = pf.getSteps().get(pf.getSteps().size() - 1);
        applyStepSupplier(pf, ((assLevel, log, reason) -> new LabeledProofStep<>(assLevel, psm.getState().get(), (ModalLogicalOperation) log, reason)));
    }

    @Override
    public boolean equals(Object o) {
        if (o instanceof ModalDiaE modalDiaE) {
            return j == modalDiaE.j;
        } else {
            return false;
        }
    }
