...
public final class ModalCopy<T> extends Copy<ModalOperation, LabeledProofStep<T>, ModalNaturalDeduction<T>> implements ModalAction<T> {
    ...
    public void apply(ModalNaturalDeduction<T> pf) {
        applyStepSupplier(pf, (assLevel, log, reason) -> new LabeledProofStep<>(assLevel, getState(pf, getAppliedAt()), (ModalLogicalOperation) log, reason));
    }

    private T getState(ModalNaturalDeduction<T> pf, int k) {
        return (pf.getSteps().get(k - 1)).getState().get();
    }
