...
public class ModalDiaE implements ModalAction {
    ...
    public boolean isValid(Proof pf) {
        ...
        if (!(pf.getSteps().get(j - 1).getStep() instanceof Sometime)) {
            return false;
        }
        ...
        ProofStep log1 = pf.getSteps().get(pf.getSteps().size() - i + 1);
        ProofStep log2 = pf.getSteps().get(pf.getSteps().size() - i);
    ...
    public void applyAbstract(Proof pf, ProofStepSupplier supp) {
        ...
        int i = 1;
        while (i <= pf.getSteps().size() && pf.getSteps().get(pf.getSteps().size() - i).getAssumptionLevel() >= psm.getAssumptionLevel()) {
            i++;
        }
        lst.add(j);
