...
- `CompileAndTest.yml`: `mvn verify` + Sonar + JaCoCo report, and a separate PIT mutation job. `frontend.yml`: `npm ci`, `npm run typecheck`, `npm test`, Sonar. `OnMerge.yml` removes a closed PR's reports from gh-pages.
