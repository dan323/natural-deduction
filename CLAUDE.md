...
Natural deduction proof assistant for classical, intuitionistic and modal propositional logic (plus modal logic with Until): Java 21 / Spring Boot backend (multi-module Maven) plus a React 19 / TypeScript / Vite frontend. `docs/` has longer prose docs (`docs/API.md` is the REST reference); when they disagree with the code or the poms, trust the code.
...
- `domain/use-cases/`: `model` (DTOs `ProofDto`/`StepDto`/`ActionDto`/`ActionDescriptorDto`), `base-use-case` (`ActionsUseCases`, `Transformer` DTO<->domain, `ProofParser` text->proof, `LogicalApplyAction`, `LogicalSolver`, the exceptions, and `ActionsUseCaseConfiguration`), plus `classical-use-case` (which also holds intuitionistic logic, package `com.dan323.uses.intuitionistic`) and `modal-use-case` (which also holds modal logic with Until, package `com.dan323.uses.modal.until`).
...
**How a new logic plugs in:** each logic's use-case module exposes `Transformer`, `ProofParser` and `LogicalGetActions` beans, keyed by a logic-name string (`"classical"`, `"intuitionistic"`, `"modal"`, `"modal-until"`). `ActionsUseCaseConfiguration` collects all beans of those types into maps keyed by `logic()`. The URL `{logic}` path segment picks the entry. So adding a logic means adding modules plus a `*Configuration` that is imported in `executable/.../ApplicationConfiguration`. A logic can also be a rule filter on top of another one, like `"intuitionistic"` (`IntuitionisticConfiguration` in `classical-use-case`): it reuses the classical language, proofs and parser, and its `Transformer`/`ProofParser`/`LogicalGetActions` subclass or filter the classical ones to drop every `AvailableAction` whose `isIntuitionistic()` is false (only `NOTE`, the `-E` rule); such a proof or action is a 400. A logic can also extend another one, like `"modal-until"` (`ModalUntilConfiguration` in `modal-use-case`): modal logic plus `A U B`. It adds `ModalUntilLogicParser` (a `ModalLogicParser` subclass through its protected constructor; `U` is an operator only as a standalone word, because javaluator splits names at operator symbols, e.g. `TRUE`), the rules in `com.dan323.proof.modal.until` (`UI [n]`, `UI [n, m]`, `UE [n]`, `UE [n, m]`, read by `ParseModalUntilAction`, which falls back to `ParseModalAction`), and a `ModalNaturalDeduction` built with that rule reader (`ParseModalUntilAction.newProof`); its `ModalUntilProofTransformer`/`ModalUntilProofParser` override the protected parse hooks of the modal ones. `ModalLogicParser`'s precedences go in steps of 10 so an extension can slot an operator in between (`U` is 35, between `->` and `-`). `"modal"` must not change: it has no `U` (there `p U q` is one variable named `p U q`, as it always was). A logic may also expose a `LogicalExercises` catalog (`GET .../exercises`; a known logic without one answers `[]`). Each `Exercise` keeps a reference solution in the proof-text layout that is never sent to clients; `ClassicalExercisesTest`/`IntuitionisticExercisesTest`/`ModalUntilExercisesTest` replay every one, so a new exercise must come with a solution that parses and is done.
...
- `GET .../actions` returns `ActionDescriptorDto`s (`name` + ordered `params` of `ParamKind` `INT`/`EXPRESSION`/`STATE`), built once at startup. Classical: `ClassicGetActions` maps every `AvailableAction` (its `switch` has no default, so a new action does not compile until described). Intuitionistic: `IntuitionisticGetActions`, the classical descriptors filtered by `AvailableAction.isIntuitionistic()`. Modal: `AvailableModalAction`, whose names are the rule names `ParseModalAction.parseAction` understands. Modal-until: the modal descriptors, then `AvailableUntilAction` (`UI1`, `UI2`, `UE1`, `UE2`); both enums build descriptors with `ActionInput`. `ActionDto.name` must equal a descriptor name. `INT` params are the 1-based `ActionDto.sources`, in order; `EXPRESSION` goes in `extraParameters.expression`, `STATE` in `extraParameters.state`. The frontend `Menu` renders its inputs from the descriptor.
...
- `OnMaster.yml` ("Publish Docker image"): typechecks and tests the frontend, embeds it, runs `mvn verify`, then builds the image from that exact jar, smoke-tests it and only then pushes to Docker Hub. The smoke test checks `/logic/classical/actions`, `/logic/modal/actions`, `/logic/intuitionistic/actions`, `/logic/modal-until/actions`, the UI and that the container user is not root.
