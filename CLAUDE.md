...
- Exercises UI: the "Exercises" button opens `ExerciseList` (from `GET /logic/{logic}/exercises`, grouped by difficulty, with a "Solved n of m" counter and a "(Solved)" marker). Starting an exercise (or "Next exercise") goes through the same in-page discard confirmation as New Proof when the current proof has more than its premises. An exercise counts as solved when the backend's `done` is true for its proof.
