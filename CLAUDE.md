...
- Proof text files (`POST .../proof`) and `ProofStep.toString()` use a fixed layout: 3 spaces of indent per assumption level, an 11-space gap, then the rule, e.g. `->I [1-2]`. `ProofParser.ProofLine`/`parseLine` and `ProofReason.parseReason` depend on this. Frontend `proofToText` (`service/utils.ts`, "Copy proof as text") writes it and `loadProofFromText` (`service/actions.ts`, "Load from text" in `NewProofModal`) reads it back with `parseProofText` (a copy of `ProofLine.split`) and replays the steps through `POST .../action`, not this endpoint, so that proofs ending in an open subproof load and the goal is the typed one. Frontend `ProofViewer` also regex-parses the `[1-2, 4]` part of rule strings for hover highlighting.
