...
- `POST .../action` answers 200 with `ProofResponse(proof, success, done, message)`, or 202 with `success=false` when the action is well formed but does not apply (proof returned unchanged). `done` is the domain's `Proof.isDone()` (a top-level step equals the goal, not necessarily the last); the UI reads it instead of computing success itself.
